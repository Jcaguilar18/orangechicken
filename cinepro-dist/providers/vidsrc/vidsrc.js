...
class VidSrcProvider extends framework_1.BaseProvider {
    id = 'vidsrc';
    name = 'VidSrc';
    enabled = true;
     ...
     */
    extractSecondUrl(html) {
        const src = html.match(/<iframe[^>]*\s+src=["']([^"']+)["'][^>]*>/i)?.[1];
        if (!src) {
            return null;
        }
        return { url: src };
     ...
     */
    extractThirdUrl(html, secondUrl) {
    ...
    extractM3u8Urls(thirdHtml) {
        const fileField = thirdHtml.match(/file\s*:\s*["']([^"']+)["']/i)?.[1];
        if (!fileField)
            return null;
        const playerDomains = new Map();
        ...
        const filteredM3u8Urls = m3u8Urls.filter((url) => url !== null);
...
exports.VidSrcProvider = VidSrcProvider;
