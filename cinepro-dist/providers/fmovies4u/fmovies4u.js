...
class Fmovies4U extends framework_1.BaseProvider {
    ...
    async getSources(media, params) {
        try {
            const path = params.type === 'movie'
                ? `/api/movie/${media.tmdbId}`
                : `/api/tv/${media.tmdbId}/${params.season}/${params.episode}`;
            const request = await fetch(`${this.BASE_URL}${path}?_cb=${Date.now()}`, {
                headers: {
                    ...this.HEADERS,
            ...
            if (!encryptedText) {
                return this.emptyResult('Failed to fetch encrypted sources', media);
            }
            const decrypted = (0, decrypt_js_1.decrypt)(encryptedText);
     ...
     */
    mapApiResponseToResult(apiResponse, media) {
        const sources = [];
        const subtitles = [];
        for (const apiSource of apiResponse.sources) {
            const streamUrls = Array.isArray(apiSource.url)
                ? apiSource.url
                : [];
            for (const stream of streamUrls) {
                const type = (stream.type?.toLowerCase() ||
                    'hls');
                sources.push({
                    url: this.createProxyUrl(stream.link, stream.link.includes('file2')
                        ? {
            ...
            if (Array.isArray(apiSource.tracks)) {
                for (const track of apiSource.tracks) {
                    if (!track || typeof track !== 'object')
                        continue;
                    const kind = (track.kind || track.type || '')
                        .toString()
                        .toLowerCase();
                    const isSubtitle = kind === 'subtitles' ||
                        kind === 'subtitle' ||
                        kind === 'captions' ||
